...
public class Game extends JFrame implements KeyListener {
	...
	private Booster booster = new Booster(400, 400, 50);
	...
	private void run() {
		while (player.getLife() > 0) {
			...
			if (player.isInvencivel()) {
				...
				if (booster.getTempoDuracao() > 0) {
					booster.setTempoDuracao(booster.getTempoDuracao() - 1);
				} else {
	...
	public void colisao() {
		int x_player = player.getPosicaox();
		int y_player = player.getPosicaoy();

		if ((x_player == ghost1.getPosicaox() && y_player == ghost1.getPosicaoy()) ||
				(x_player == ghost2.getPosicaox() && y_player == ghost2.getPosicaoy()) ||
				(x_player == ghost3.getPosicaox() && y_player == ghost3.getPosicaoy()) ||
				(x_player == ghost4.getPosicaox() && y_player == ghost4.getPosicaoy())) {
	...
	public void colisaoBomb() {
		...
		if (x_player == bomb.getPosicaox() && y_player == bomb.getPosicaoy() && bomb.isVisivel()) {
			player.setLife(player.getLife() - 1);
	...
	public void colisaoBooster() {
		...
		booster.setTempoDuracao(aleatorio.nextInt((30 - 10) + 1) + 10);

		int x_player = player.getPosicaox();
		int y_player = player.getPosicaoy();

		if (x_player == booster.getPosicaox() && y_player == booster.getPosicaoy() && booster.isVisivel()) {
			player.setInvencivel(true);
			booster.setVisivel(false);
			imgBooster.setVisible(false);
			JOptionPane.showMessageDialog(this, "Booster!");
		}
	}
